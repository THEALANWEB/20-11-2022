...
//ALO 1-2-3-4 This function returns a list of numbers of hearts, reading from left to right in the database
//So the current order is Thanh Ngọc, Yến Nhiên, Gia Anh, Quỳnh 
function getNumberOfHearts()
{
    if (finishedGettingInfo == false)
    {
        console.log("The data hasn't been fetched yet, please try again at a later time!");
    }
    else
    {
...
//Update the value in the given cell ("location" parameter)
//Example: updateNumberOfHearts(100, "E3")
function updateNumberOfHearts(number = 0, location = "")
{
    write("Data!" + location, [[number]]);
}
...
async function setUp() 
{
    ...
    await read("Data!B3:E3").then((req) => {
        finishedGettingInfo = true;
        numberOfHearts = req["values"];
    })
}
